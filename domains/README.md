...
- **free** - object-level build/ship/assurance work *acts on* a surface, so the
  ...
  distribution` != `... x authority-governance`). D1/D2/D3 anchor this.
...
D4/D5/D6 remain first-class domains (not deletions) because they are still valid **free** targets of `audit-a-surface` / `verify-gate` / `design-a-contract` (you can audit the WorkGraph, verify the governance substrate, design a knowledge contract).\
...
When a fix in one surface serves another (e.g. a `tooling-harness` prompt-table fix that exists to serve `distribution`), **the evidence-target wins**: the domain is where the node's diff/evidence resolves.\
