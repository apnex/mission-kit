...
Add a test that pins a real behavior on an under-tested surface - a guard that turns red on regression or a falsifier that mutation-proves it can fail.\
The value is the failing observation the test is capable of producing, not the green tick.
...
`proactive-poolable` - mintable against the existing substrate with no trigger, so it enters the idle-QoS pool (idea-403/404). idea-425/451/403 instantiate it as an idle engineer picking an under-tested surface, minting the node with `target` bound from substrate discovery, and closing provisionally under a bracketing verify-gate.\
The falsifier is **a vacuous test that cannot fail** - a test whose mutation proof is absent (nothing it asserts can go red) fails the seed's constraint-5/7 preflight rather than closing as assurance.
