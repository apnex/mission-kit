...
Land an already-approved PR onto canonical `main` - the git-plane act of integrating a reviewed branch, distinct from the publish/deploy release plane.
...
`reactive-triggered`: instantiated when a PR reaches approved/mergeable state (idea-425/451/403 mint it off that substrate trigger - an approved PR - rather than pooling it idle).\
The falsifier is concrete: *branch not merged to canonical main* turns the node FAIL rather than leaving prose open.
