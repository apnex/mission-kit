...
Hold a standing abort/rollback authority over a bracketed risk window (a deploy, a migration, a prod change), releasing only once every bracketed node has closed.\
Not a role and not an overlay layer - an ordinary `arc-seeded` work-type carrying a `backstop:true` flag.
...
`arc-seeded` - minted by a driver inside a blueprint that brackets a risk window, with `window` provided by the trigger. idea-425/451/403 instantiate it as the backstop rung a seed/drive/closeout arc plants alongside the risky nodes; its `completionDependsOn` on those nodes is the "stands-down-last" edge, so the engine cannot terminal-close the backstop while any bracketed node is live.\
Falsifier: it stood down before closeout, or an abort trigger fired and was ignored.
