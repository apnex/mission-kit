...
An author working from the spec alone produces an instance that is *valid* but *foreign* - right sections, wrong voice; right fields, wrong altitude; project-specific detail where peers keep it quarantined.\
Each foreign instance then becomes a bad exemplar for the next author, compounding drift.\
The failure is invisible at authoring time (the spec is satisfied) and expensive at review time (a reviewer must now articulate conventions nobody wrote down).\
Reading one peer costs minutes; it converts every unwritten convention into free guidance and keeps the collection coherent enough that its own instances remain trustworthy exemplars.
