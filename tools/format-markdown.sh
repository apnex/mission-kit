...
#
#   S13  non-ASCII converted using the table the entry publishes
...
for f in "${files[@]}"; do
	...
	perl -CSD -i -pe '
		s/ \x{2014} / - /g; s/\x{2014}/-/g; s/\x{2013}/-/g;
		s/\x{2192}/->/g; s/\x{2190}/<-/g; s/\x{2194}/<->/g;
		s/\x{21D2}/=>/g; s/\x{27FA}/<=>/g; s/\x{27F6}/-->/g;
		s/\x{2265}/>=/g; s/\x{2264}/<=/g; s/\x{2260}/!=/g; s/\x{2248}/~=/g;
		s/\x{00D7}/x/g; s/\x{2212}/-/g; s/\x{2026}/.../g;
		s/\x{2019}/\x27/g; s/[\x{201C}\x{201D}]/"/g;
		s/ \x{00B7} / - /g; s/\x{00B7}/-/g;
		s/\x{00A7}\s*/section /g;
	' "$tmp"
