...
Comments inside code blocks (`# ...`, `// ...`, `-- ...`) explain what an
adjacent code line does. They are NOT a substitute for prose paragraphs.
Multi-line explanatory text, side notes, flag descriptions, design rationale,
or "here's some context about this workflow" all belong in prose **between**
...
**Prose is searchable + indexable + screen-readable.** A paragraph between
...
X` above a `sudo ./scripts/remove.sh` line, they have to mentally
disambiguate: is this comment telling me to ADD that flag (i.e., modify the
command), or is it just listing flags I could optionally use? Prose makes
that distinction visible by separating the executable from the explanatory.

**Doc-block tooling.** AI doc-parsers, syntax highlighters, copy-on-click
buttons all work on the boundary "this is a code block." Prose hidden in a
code block as comments is invisible to that tooling.

**Copy-paste hygiene.** An operator copying the code block expects to get
