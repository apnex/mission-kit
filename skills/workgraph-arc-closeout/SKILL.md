...
metadata:
  ...
  related-skills: arc-lifecycle, workgraph-arc-operator, workgraph-arc-participant, workgraph-pr-delivery, workgraph-verification-gates, workgraph-recovery
  series: workgraph
...
- verifier gate WorkItems or attestations, including fresh `verify_attestation` results for every load-bearing current PASS and preserved immutable FAIL/repair lineage;
...
- a failed verifier gate is hidden, replayed, reinterpreted, or lacks distinct repair lineage;
- a current load-bearing PASS was not fresh-validated with `verify_attestation` and exact candidate/target evidence;
- authority/manifest/constitution/currentness bindings required by the closeout fence are missing, stale, or mismatched;
