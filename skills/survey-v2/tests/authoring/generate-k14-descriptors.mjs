...
function packagePath(relativePath) {
  if (
    ...
    fail(`unsafe K14 package path: ${String(relativePath)}`);
  }
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) {
    fail(`K14 path escapes package root: ${relativePath}`);
  }
  return target;
}
...
const discovered = (await readdir(packagePath(testRoot), {
  withFileTypes: true,
}))
  .filter(
    (entry) =>
  ...
  .sort(compareUtf8);
const catalog = k14Suite
  .map(({ executable }) => executable)
  .sort(compareUtf8);
if (JSON.stringify(discovered) !== JSON.stringify(catalog)) {
  fail(
    `K14 catalog/test inventory diverges: discovered=${JSON.stringify(discovered)} catalog=${JSON.stringify(catalog)}`,
  );
}
if (
  new Set(k14Suite.map(({ obligationId }) => obligationId))
    .size !== k14Suite.length
) {
  fail("K14 suite contains a duplicate obligation");
}

for (const item of k14Suite) {
  ...
  const descriptor = {
    ...
    verification: {
      precondition:
        "The K14 Brief fixture, mapped generic authorities, and immutable fixtures exist as package-owned files.",
      stimulus:
        `Execute ${item.executable} with node:test under its declared adapter applicability.`,
      ...
      forbiddenMutation:
        "Do not infer Survey integration, public API stability, filesystem durability, revisions, external coupling, or behavior beyond the literal assertion.",
