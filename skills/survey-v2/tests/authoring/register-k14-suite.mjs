...
function packagePath(relativePath) {
  if (
    ...
    fail(`unsafe K14 registration path: ${String(relativePath)}`);
  }
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) {
    fail(`K14 registration path escapes package root: ${relativePath}`);
  }
  return target;
}
...
for (const item of k14Suite) {
  ...
  if (
    ...
    fail(
      `${descriptorPath} differs from its K14 suite authority`,
    );
  }
...
for (const item of k14Suite) {
  ...
  if (!invariant) {
    fail(`K14 suite names missing invariant ${item.invariantId}`);
  }
...
const newMembers = [
  "tests/authoring/generate-k14-descriptors.mjs",
  "tests/authoring/k14-suite-definition.mjs",
  "tests/authoring/register-k14-suite.mjs",
  ...await discover("tests/authoring/non-survey-brief"),
  ...await discover(
    "tests/fixtures/authoring/non-survey-brief",
  ),
];
