...
test("evidence replay rejects rewritten raw Submission bytes under the same semantic reference", async () => {
    ...
    handlerInvoke(input) {
      ...
      return {
        ...
        issues: [{
          ...
          field: "/summary",
          reason: "The fixture retains this attempt.",
          correction: "Submit corrected evidence.",
        }],
      };
    },
  });
  ...
  const resealed = fullyResealRawSubmissionRewrite(
    snapshot,
    Buffer.from("rewritten raw evidence\n", "utf8"),
  );
  ...
  assert.notEqual(
    resealed.evidencePlan.mutationDigest,
    resealed.original.mutationDigest,
  );
  assert.equal(
    resealed.evidencePlan.mutationDigest,
    evidenceMutationDigest(resealed.evidencePlan),
  );
  assert.equal(
