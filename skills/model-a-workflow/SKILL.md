...
Not for: the *states* a thing is in and the events between them (that's `model-a-state-machine` — a `state
def`); or static structure with no behaviour (that's `model-a-component` — a `part def`).
...
semantics by hand — the parser will *not* catch an unreachable action or an unbound pin. The decidable
workflow rules are written as a checkable spec in **[`assets/well-formedness.sysml`](assets/well-formedness.sysml)**.
