...
| `axiom_alignment_audit` | Scope fence and current constitution | `dependsOn: [scope_fence]`; runbook requires `get_constitution` / `get_axiom` provenance and A1-A14 mapping or explicit not-required rationale | Direct axiom mapping is produced before implementation authority and before design options can finalize. |
