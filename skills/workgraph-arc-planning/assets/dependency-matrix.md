...
| `design_options` | Scope fence, inventory, verifier audit | `dependsOn: [scope_fence, current_state_inventory, failure_mode_audit]` | Options evidence must compare shapes after engineer and verifier inputs exist. |
...
| `design_gate` | Design options, feasibility sketch, failure-mode audit | `dependsOn: [design_options, feasibility_sketch, failure_mode_audit]` | Gate runbook must check concrete artifacts, encoded dependencies, validation, closeout/survey proof, active-surface boundaries, and anti-scope. |
| `final_design_packet` | Design gate, design options, feasibility sketch | `dependsOn: [design_gate, design_options, feasibility_sketch]` | Final packet cannot be claimable until verifier gate and source design inputs are done. |
...
```text
PASS planning blueprint validation: 11 nodes, 9 dependency assertions, driver gates 10 children, negative checks caught 3 broken variants.
```
