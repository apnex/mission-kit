...
status: active
hydrate-when: You are about to claim authority that only the director holds
...
## Essence
The sovereign intent-source and ratification authority - the delegator whose authority the agentic roles act under.\
Distinct from the other three: the Director sets intent and ratifies, rather than claiming and executing work-nodes.
...
`composing: false`.\
Director does not participate symmetrically in `roleEligibility` unions, is never idle-poolable, and is the sole `director-ratification` authority.\
It is a first-class role by charter mandate and A13, but is honestly marked non-composing so the generation engine never attempts to pool or auto-mint director work.\
Director-mode work-types carry `generationMode: externally-triggered`.
