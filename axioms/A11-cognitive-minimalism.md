...
## Mandate
LLM tokens are the scarce economic resource.\
Every deterministic function is mechanized; the LLM is invoked only for genuinely cognitive work - judgment, creativity, ambiguity resolution.\
Maximum logic-per-token is the engineering objective.
...
## Rationale
LLM token consumption is the dominant variable cost of an agentic system and its primary scarce resource.\
Without a first principle that drives deterministic work out of the prompt, agents silently absorb toil that could be mechanized - paying in tokens what one function call would do for free, and burning context-window on ceremony that displaces judgment.\
Naming the principle promotes each local efficiency fix from an ad-hoc workaround to a governing rule.\
